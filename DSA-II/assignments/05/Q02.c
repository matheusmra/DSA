...
int main() {
    ...
    /* Ler ids da entrada padrão e construir vetor de jogos selecionados */
    ...
    /* Se não houver elementos selecionados, limpa e sai */
    if (nSel == 0) {
    ...
    for (int i = 0; i < nSel - 1; i++) {
        ...
        for (int j = i + 1; j < nSel; j++) {
            comparacoes++;
            if (strcmp(selecionados[j].name, selecionados[minIndex].name) < 0) {
                minIndex = j;
            }
        }
        if (minIndex != i) {
            Game tmp = selecionados[i]; movimentacoes++;
            selecionados[i] = selecionados[minIndex]; movimentacoes++;
            selecionados[minIndex] = tmp; movimentacoes++;
        }
    }
    ...
    FILE *log = fopen("848813_selecao.txt", "w");
    if (log) {
