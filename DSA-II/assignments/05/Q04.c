...
int dateKey(const char *date) {
    if (!date || date[0] == '\0') return 19000101;
    int d = 1, m = 1, y = 1900;
    if (sscanf(date, "%d/%d/%d", &d, &m, &y) >= 1) {

    }
    return y * 10000 + m * 100 + d;
}

// compara por releaseDate (DD/MM/YYYY) e desempata por appID; incrementa compCount a cada comparação
int comparar(const Game *a, const Game *b, long long *compCount) {
    if (compCount) (*compCount)++;
    int ka = dateKey(a->releaseDate);
    int kb = dateKey(b->releaseDate);
    if (ka < kb) return -1;
    if (ka > kb) return 1;
...
void swap(Game *x, Game *y, long long *movCount) {
    Game tmp = *x; 
    if (movCount) (*movCount)++;
    *x = *y; 
    if (movCount) (*movCount)++;
    *y = tmp; 
    if (movCount) (*movCount)++;
...
void quicksort(Game arr[], int esq, int dir, long long *compCount, long long *movCount) {
    if (esq >= dir) return;
    int i = esq, j = dir;
    Game pivo = arr[(esq + dir) / 2];
    while (i <= j) {
        while (comparar(&arr[i], &pivo, compCount) < 0) i++;
        while (comparar(&arr[j], &pivo, compCount) > 0) j--;
        if (i <= j) {
            swap(&arr[i], &arr[j], movCount);
            i++; j--;
        }
    }
    if (esq < j) quicksort(arr, esq, j, compCount, movCount);
    if (i < dir) quicksort(arr, i, dir, compCount, movCount);
}
...
int main() {
    ...
    Game *selecionados = malloc(sizeof(Game) * qtd);
    if (!selecionados) { fprintf(stderr, "Memória insuficiente para selecionados\n"); liberarRecursos(jogos, qtd); free(jogos); return 1; }
    int nSel = 0;
    char entrada[64];
    while (1) {
    ...
    quicksort(selecionados, 0, nSel - 1, &comparacoes, &movimentacoes);

    clock_t fim = clock();
    long tempo = (long)((fim - comeco) * 1000 / (double)CLOCKS_PER_SEC);

    for (int i = 0; i < nSel; i++) {
        imprimirJogo(&selecionados[i]);
    }
    FILE *log = fopen("848813_quicksort.txt", "w");
    if (log) {
        fprintf(log, "848813\t%lld\t%lld\t%ld", comparacoes, movimentacoes, tempo);
        fclose(log);
    } else {
