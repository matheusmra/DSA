...
// Estrutura para célula da lista encadeada
typedef struct Celula {
    Game *game;
    struct Celula *prox;
} Celula;

// Estrutura para a tabela hash indireta
typedef struct {
...
/**
 * Função hash: (soma ASCII do Name) mod TAM_TAB
 */
 ...
 */
void inicializarHash(HashIndireto *h) {
...
/**
 * Insere um jogo na tabela hash indireta (no início da lista)
 */
void inserirHash(HashIndireto *h, Game *game) {
    int pos = hash(game->name);
    
    Celula *nova = (Celula *)malloc(sizeof(Celula));
    nova->game = game;
    nova->prox = h->tabela[pos];
    h->tabela[pos] = nova;
}

/**
 * Pesquisa um jogo na tabela hash pelo nome
 * Retorna 1 se encontrado, 0 caso contrário
 */
int pesquisarHash(HashIndireto *h, const char *nome) {
    int pos = hash(nome);
 ...
 */
int getPosicaoHash(const char *nome) {
    return hash(nome);
}
 ...
 */
void liberarHash(HashIndireto *h) {
    for (int i = 0; i < TAM_TAB; i++) {
 ...
 */
Game* buscarPorId(Game *jogos, int qtd, int id) {
    for (int i = 0; i < qtd; i++) {
        if (jogos[i].appID == id) {
            return &jogos[i];
        }
...
int main() {
    ...
    while (1) {
        ...
        Game *gameEncontrado = buscarPorId(jogos, qtd, id);
        if (gameEncontrado != NULL) {
            inserirHash(&tabela, gameEncontrado);
        }
    }
    ...
    while (1) {
        ...
        int posHash = getPosicaoHash(entrada);
        int encontrado = pesquisarHash(&tabela, entrada);
    ...
    clock_t fim = clock();
    long tempoMs = (long)((fim - inicio) * 1000 / CLOCKS_PER_SEC);

    // Gerar arquivo de log
    FILE *log = fopen("848813_hashIndireta.txt", "w");
    if (log) {
        fprintf(log, "848813\t%d\t%ld\n", totalComparacoes, tempoMs);
        fclose(log);
    }
