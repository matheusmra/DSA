...
class ArvoreBinaria {
    ...
    public ListaStr pesquisarCaminho(String nome) {
        ...
        // Não inserir "raiz" aqui - será tratado na árvore principal
        while (true) {
            comparacoes++;
...
class No1 {
    public int chave;
    public ArvoreBinaria raizSec;
    public No1 esq, dir;

    public No1(int chave) {
        this.chave = chave;
        this.raizSec = new ArvoreBinaria();
        this.esq = this.dir = null;
    }
}
...
class ArvoreDeArvores {
    ...
    private No1 inserirChave(No1 i, int chave) {
        if (i == null) {
            return new No1(chave);
        }
        if (chave < i.chave) {
            i.esq = inserirChave(i.esq, chave);
        } else if (chave > i.chave) {
            i.dir = inserirChave(i.dir, chave);
        }
        return i;
    }

    public No1 buscarNo(int chave) {
        return buscarNo(raiz, chave);
    }

    private No1 buscarNo(No1 i, int chave) {
        if (i == null) return null;
        if (chave == i.chave) return i;
        if (chave < i.chave) return buscarNo(i.esq, chave);
        else return buscarNo(i.dir, chave);
    }

    public void inserirGameEmSecundaria(Game g) {
        int chave = 0;
        if (g != null) chave = g.getGameEstimatedOwners() % 15;
        No1 n = buscarNo(chave);
        if (n != null) {
            n.raizSec.inserir(g);
        }
    }
    ...
    /**
     * Pesquisa o nome fazendo um "mostrar" (percurso) na primeira árvore.
     * Para cada nó visitado, faz um "mostrar" (pesquisa) na árvore secundária.
     */
    public PesquisaResultado pesquisar(String nome) {
        encontrado = false;
        StringBuilder resultado = new StringBuilder();
        comparacoes = 0;
        
        if (raiz == null) {
            return new PesquisaResultado(false, resultado.toString());
        }
        
        // Iniciar com "raiz"
        resultado.append("raiz");
        
        // Fazer mostrar (pre-ordem) na primeira árvore
        mostrarPrimeira(raiz, nome, resultado);
        
        return new PesquisaResultado(encontrado, resultado.toString());
    }
    
    private void mostrarPrimeira(No1 node, String nome, StringBuilder resultado) {
        if (node == null || encontrado) return;
        
        // Pesquisar na árvore secundária deste nó
        if (node.raizSec != null) {
            node.raizSec.resetComparacoes();
            ListaStr caminhoSec = node.raizSec.pesquisarCaminho(nome);
            comparacoes += node.raizSec.getComparacoes();
            
            boolean achou = node.raizSec.getUltimoResultado();
            
            // Adicionar caminho da secundária (em minúsculas)
            try {
                for (int i = 0; i < caminhoSec.tamanho(); i++) {
                    String passo = caminhoSec.get(i).trim();
                    if (!passo.isEmpty()) {
                        resultado.append(" ").append(passo.toLowerCase());
                    }
                }
        ...
        // Se não encontrou, continuar para esquerda
        if (!encontrado && node.esq != null) {
            resultado.append("  ESQ");
            mostrarPrimeira(node.esq, nome, resultado);
        }
        
        // Se não encontrou, continuar para direita
        if (!encontrado && node.dir != null) {
            resultado.append("  DIR");
            mostrarPrimeira(node.dir, nome, resultado);
        }
    }
...
class PesquisaResultado {
    ...
    public PesquisaResultado(boolean encontrado, String caminho) {
...
public static void main(String[] args) {
    ...
    // Construir a primeira árvore com as chaves na ordem especificada
    int[] ordem = {7, 3, 11, 1, 5, 9, 13, 0, 2, 4, 6, 8, 10, 12, 14};
    for (int k : ordem) arv.inserirChave(k);

    // Carregar todos os jogos do CSV com codificação UTF-8
    try {
        BufferedReader br = new BufferedReader(new InputStreamReader(
                new FileInputStream("/tmp/games.csv"), "UTF-8"
        ));
    ...
    try (Scanner scanner = new Scanner(System.in, "UTF-8")) {
        ...
        // Primeira parte: inserir jogos nas árvores secundárias
        while (true) {
            ...
            try {
                ...
                if (g != null) arv.inserirGameEmSecundaria(g.clone());
        ...
        while (scanner.hasNextLine()) {
            ...
            PesquisaResultado res = arv.pesquisar(nome);

            // O caminho já vem formatado corretamente
            String out = res.caminho;

            out += (res.encontrado ? "  SIM" : "  NAO");

            // impressão final correta
