...
No *rotacaoDir(No *y)
{
    ...
    x->dir = y;
    y->esq = T2;
    y->altura = maxInt(alturaNo(y->esq), alturaNo(y->dir)) + 1;
...
/* Insere por nome; se já existir (strcmp == 0) não insere */
No *inserirAVL(No *node, Game g)
{
    if (!node)
        return novoNo(g);

    int cmp = strcmp(g.name, node->g.name);
    if (cmp < 0)
        node->esq = inserirAVL(node->esq, g);
    else if (cmp > 0)
        node->dir = inserirAVL(node->dir, g);
    else
        return node; /* não insere duplicata */

    node->altura = 1 + maxInt(alturaNo(node->esq), alturaNo(node->dir));

    int balance = fatorBalanceamento(node);

    if (balance > 1 && strcmp(g.name, node->esq->g.name) < 0)
        return rotacaoDir(node);
    if (balance < -1 && strcmp(g.name, node->dir->g.name) > 0)
...
/* Busca que imprime caminho e retorna se achou, conta comparações via ponteiro */
int comparacoes = 0;
int buscarImprimirCaminho(No *root, const char *key)
{
    if (!root)
    {
        printf("raiz NAO\n");
    ...
    while (atual)
    {
        ...
        int cmp = strcmp(key, atual->g.name);
        comparacoes++;
        if (cmp == 0)
        {
            printf(" SIM\n");
...
int main()
{
    ...
    while (fgets(linha, sizeof(linha), stdin))
    {
        ...
        int id = stringParaInt(linha);
        for (int i = 0; i < qtd; i++)
        {
            if (jogos[i].appID == id)
            {
                raiz = inserirAVL(raiz, jogos[i]);
                break;
            }
        }
    }
    ...
    else
    {
        ...
        if (isNumber && linha[0] != '\0')
        {
            nbuscas = stringParaInt(linha);
            for (int i = 0; i < nbuscas; i++)
            {
                ...
                printf("%s: ", linha);
                buscarImprimirCaminho(raiz, linha);
            }
        }
        else
        {
            if (strcmp(linha, "FIM") != 0 && linha[0] != '\0')
            {
                printf("%s: ", linha);
                buscarImprimirCaminho(raiz, linha);
            }
    ...
    long t_end = clock();
    double tempo = (double)(t_end - t_start) / CLOCKS_PER_SEC;
    FILE *logf = fopen("848813_arvoreAvl.txt", "w");
    if (logf)
    {
        fprintf(logf, "848813 %.6f %d\n", tempo, comparacoes);
        fclose(logf);
    }
